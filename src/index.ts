  ...
  inputSchema: {
    ...
    properties: {
      ...
      system: {
        ...
        minLength: 1,
        description:
          '问题所属系统名称（必填，如：VOSA、财务系统、OA系统、CRM等）'
      },
      eventId: {
        type: 'string',
      ...
      title: {
        ...
        maxLength: 100,
        description:
          '工单标题（必填，简明扼要描述问题，如："登录页面无法访问"、"审批流程卡住"）'
      },
      content: {
        type: 'string',
        minLength: 1,
...
- 发生时间：问题首次出现的时间`
      ...
      attachments: {
        ...
        items: {
          ...
          properties: {
            filename: {
              type: 'string',
              description:
                '附件文件名（必须含扩展名，如：error.log、screenshot.png）'
            },
            url: {
              type: 'string',
              description:
                '附件访问URL（需可公网访问或内网可达，支持 http/https 协议）'
            },
            size: {
              type: 'number',
        ...
        description:
          '附件列表（可选，支持截图、日志、录屏等，建议单个文件不超过10MB）'
      }
    }
  }
}
...
// ============ 全局配置 ============
let ESB_URL: string = process.env.ESB_URL || ''
 ...
 */
function buildESBPayload(input: FeedbackInput): string {
 ...
 */
async function createWorkOrder(input: FeedbackInput): Promise<string> {
  const url = ESB_URL
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `ESB API request failed: ${response.status} ${response.statusText}\nResponse: ${errorText}`
      )
    }

    const responseData: ESBResponse = await response.json()
 ...
 */
function validateInput(args: unknown): asserts args is FeedbackInput {
  if (!args || typeof args !== 'object') {
    throw new Error('Invalid input: arguments must be an object')
  }

  const input = args as Partial<FeedbackInput>
  ...
  if (
    ...
    throw new Error(
      'Invalid input: empNo is required and must be a non-empty string'
    )
  }
  ...
  if (
    ...
    throw new Error(
      'Invalid input: system is required and must be a non-empty string'
    )
  }
  ...
  if (
    ...
    throw new Error(
      'Invalid input: content is required and must be a non-empty string'
    )
  }
  ...
  if (
    ...
    throw new Error(
      'Invalid input: name must be a string with max 100 characters'
    )
  }
  ...
  if (input.attachments !== undefined) {
    ...
    for (const [index, attachment] of input.attachments.entries()) {
      if (!attachment || typeof attachment !== 'object') {
        throw new Error(
          `Invalid input: attachment at index ${index} must be an object`
        )
      }
      ...
      if (
        ...
        throw new Error(
          `Invalid input: attachment at index ${index} must have a valid filename`
        )
      }
      ...
      if (
        ...
        throw new Error(
          `Invalid input: attachment at index ${index} must have a valid url`
        )
      }
      ...
      try {
        ...
        throw new Error(
          `Invalid input: attachment at index ${index} has an invalid URL format`
        )
      }
      ...
      if (
        ...
        throw new Error(
          `Invalid input: attachment at index ${index} size must be a positive number`
        )
      }
    }
  }
...
async function runServer(): Promise<void> {
  try {
    ...
    // 输出启动日志
    console.error(
      `🚀 ${SERVER_CONFIG.name} v${SERVER_CONFIG.version} is running`
    )
    ...
    if (ESB_URL) {
      ...
      console.error(
        `⚠️  ESB URL not configured. Please set ESB_URL environment variable.`
      )
    }
  } catch (error) {
